...
from .evaluate import evaluate, evaluate_dann, evaluate_predictions
from .predict import predict, predict_dann
from .train import train, train_dann
...
def run_training_dann(args: TrainArgs, logger: Logger = None) -> List[float]:
    """
    Trains a model and returns test scores on the model checkpoint with the highest validation score.
    ...
    for model_idx in range(args.ensemble_size):
        ...
        for epoch in trange(args.epochs):
            ...
            n_iter = train_dann(
                model=model,
                data_loader_train=train_data_loader,
